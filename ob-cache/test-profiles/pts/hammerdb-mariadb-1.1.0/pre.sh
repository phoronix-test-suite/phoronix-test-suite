...
cd $HOME/mysql_
if [ "$(whoami)" == "root" ] ; then
    $HOME/mysql_/bin/mysqld_safe --no-defaults --user=root --datadir=$HOME/mysql_/.data > server.log 2>&1 &
else
    $HOME/mysql_/bin/mysqld_safe --no-defaults --datadir=$HOME/mysql_/.data > server.log 2>&1 &
fi
