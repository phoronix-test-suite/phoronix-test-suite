...
then
	cd punqs3
	rm -f PUNQS3.SMSPEC
	nice mpirun \$MPIRUN_AS_ROOT_ARG -np \$NPROC \$MPI_MAP_BY --report-bindings \$HOSTFILE ../opm-simulators/build/bin/flow --parsing-strictness=low PUNQS3.DATA >> \$LOG_FILE 2>&1
	echo \$? > ~/install-exit-status
