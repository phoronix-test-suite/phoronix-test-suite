...
MOZJS_VERSION=`ls /usr/include/mozjs-*/jsapi.h | grep "mozjs-" | sort -r | head -n1 | cut -d '-' -f2 | cut -d '/' -f1`
case $MOZJS_VERSION in
    ''|*[!0-9]*) MOZJS_VERSION=78 ;;
    *) echo "MOZJS_VERSION = $MOZJS_VERSION" ;;
esac

./configure --disable-docs --disable-fauxton --spidermonkey-version $MOZJS_VERSION 
...
cd ~
echo "#!/bin/sh
cd apache-couchdb-3.3.2/rel/couchdb
./bin/couchdb &
