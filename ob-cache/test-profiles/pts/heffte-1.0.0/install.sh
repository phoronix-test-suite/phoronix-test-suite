...
cd ~
echo "#!/bin/sh
cd heffte-2.3.0/build/benchmarks
OMP_NUM_THREADS=\$CPU_THREADS_PER_CORE mpirun --allow-run-as-root -np \$NUM_CPU_PHYSICAL_CORES speed3d_\$@ -n50 > \$LOG_FILE
