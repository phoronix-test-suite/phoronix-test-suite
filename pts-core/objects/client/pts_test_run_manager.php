				...
				if(!empty($test_identifier))
				{
					...
					if($test_run_request->secondary_linked_results != null && is_array($test_run_request->secondary_linked_results))
					{
						foreach($test_run_request->secondary_linked_results as &$run_request_minor)
						{
							...
							$test_run_request->test_result_buffer->add_test_result($this->results_identifier, $run_request_minor->active->get_result(), $run_request_minor->get_values_as_string(), self::process_json_report_attributes($test_run_request),$run_request_minor->get_min_result(), $run_request_minor->get_max_result());
							$this->result_file->add_result($test_run_request);
						}
					}
