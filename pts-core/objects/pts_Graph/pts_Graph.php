...
abstract class pts_Graph
{
	...
	protected function render_graph_heading($with_version = true)
	{
		...
		if($this->i['iveland_view'])
		{
			...
			$this->svg_dom->add_element('image', array('xlink:href' => pts_svg_dom::embed_png_image(PTS_CORE_STATIC_PATH . 'images/pts-logo-77x40-white.png'), 'x' => ($this->i['graph_left_end'] - 77), 'y' => (($this->i['top_heading_height'] / 40 + 2)), 'width' => 77, 'height' => 40));
		}
		else
		{
	...
	protected function render_graph_post()
	{
		if($this->i['iveland_view'])
		{
			...
			if($this->link_alternate_view)
			{
				...
				$this->svg_dom->add_element('image', array('xlink:href' => pts_svg_dom::embed_png_image(PTS_CORE_STATIC_PATH . 'images/ob-10x16.png'), 'x' => 4, 'y' => ($bottom_heading_start + 1), 'width' => 10, 'height' => 16));
			}
