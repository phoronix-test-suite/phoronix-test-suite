...
class pts_BarGraph extends pts_Graph
{
	...
	protected function render_graph_bars()
	{
		...
		$highlight_bar = PTS_MODE == "CLIENT" ? pts_client::read_env("GRAPH_HIGHLIGHT") : false;
		...
		for($i_o = 0; $i_o < $bar_count; $i_o++)
		{
			...
			foreach(array_keys($this->graph_data[$i_o]) as $i)
			{
				...
				$this->graph_image->draw_rectangle_with_border($px_bound_left + 1, $value_plot_top, $px_bound_right - 1, $this->graph_top_end, $highlight_bar == $this->graph_identifiers[$i] ? $this->graph_color_alert : $paint_color, $this->graph_color_body_light, $title_tooltip);
