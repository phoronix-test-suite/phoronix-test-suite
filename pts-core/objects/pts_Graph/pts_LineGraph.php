...
class pts_LineGraph extends pts_Graph
{
	...
	protected function render_graph_key()
	{
		...
		$group_attributes = array('onmouseover' => 'opacifyOthers(evt)', 'onmouseout' => 'resetOpacities()');
		for ($i = 0; $i < count($this->graph_data_title); ++$i)
		{
			array_push($this->dom_groups, $this->svg_dom->create_group($group_attributes));
		}
	...
	protected function renderGraphLines()
	{
		...
		// add the java script for the poly lines
		pts_LineGraph::addPolylineOpacityJavaScriptToSvg($this->svg_dom);
	...
	protected function draw_graph_line_process($group_el, &$poly_points, &$paint_color, &$regression_plots, $point_counter)
	{
		...
		$this->svg_dom->add_element_to_element($group_el, 'polyline', array('points' => $svg_poly, 'fill' => 'none', 'stroke' => $paint_color, 'stroke-width' => 1));
		...
		foreach($poly_points as $i => $x_y_pair)
		{
			...
			if($x_y_pair[3] > 0)
			{
				...
				{
					$this->svg_dom->draw_svg_line_to_element($group_el, $x_y_pair[0], $x_y_pair[1] + $std_error_rel_size, $x_y_pair[0], $x_y_pair[1] - $std_error_rel_size, $paint_color, 1);
					$this->svg_dom->draw_svg_line_to_element($group_el, $x_y_pair[0] - $std_error_width, $x_y_pair[1] - $std_error_rel_size, $x_y_pair[0] + $std_error_width, $x_y_pair[1] - $std_error_rel_size, $paint_color, 1);
					$this->svg_dom->draw_svg_line_to_element($group_el, $x_y_pair[0] - $std_error_width, $x_y_pair[1] + $std_error_rel_size, $x_y_pair[0] + $std_error_width, $x_y_pair[1] + $std_error_rel_size, $paint_color, 1);
			...
			{
				$this->svg_dom->draw_svg_line_to_element($group_el, $x_y_pair[0], $x_y_pair[1] + 6, $x_y_pair[0], $x_y_pair[1] - 6, self::$c['color']['alert'], 4, array('xlink:title' => $regression_plots[$i]));
			}

			if($point_counter < 6 || $plotted_error_bar || $i == 0 || $i == ($poly_points_count  - 1))
			{
				$this->svg_dom->add_element_to_element($group_el, 'ellipse', array('cx' => $x_y_pair[0], 'cy' => $x_y_pair[1], 'rx' => 3, 'ry' => 3, 'fill' => $paint_color, 'stroke' => $paint_color, 'stroke-width' => 1, 'xlink:title' => $x_y_pair[2]));
			}
		}
	...
	private static function addPolylineOpacityJavaScriptToSvg($svg_dom)
	{
		$line_graph_js = <<<EOD
			function opacifyOthers(evt)
			{
				// set the opacity of all polylines
				setAllPolylineOpacity(0.0);

				// set the polyline in this group as opacity 1.0
				var polyline = getPolylineNode(evt.target.parentNode);
				polyline.setAttribute("opacity", 1.0);
			}
			function resetOpacities()
			{
				// set the opacity of all polylines
				setAllPolylineOpacity(1.0);
			}
			function setAllPolylineOpacity(opacity)
			{
				var allPolyLines = document.getElementsByTagName("polyline");

				for (var i = 0; i < allPolyLines.length; ++i)
				{
					allPolyLines[i].setAttribute("opacity", opacity);
				}
			}
			function getPolylineNode(groupNode)
			{
				var child = groupNode.firstChild;

				while (child != null && child.nodeName != "polyline")
				{
...
EOD;
		...
		$script_el = $svg_dom->add_element("script", array('type' => 'text/javascript'));
		$svg_dom->add_cdata_element_to_element($script_el, $line_graph_js);
	}
}
