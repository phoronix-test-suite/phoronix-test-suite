	...
	protected function render_graph_bars()
	{
		...
		for($i_o = 0; $i_o < $bar_count; $i_o++)
		{
			...
			foreach(array_keys($this->graph_data[$i_o]) as $i)
			{
				...
				$value_end_left = max($this->c['pos']['left_start'] + round(($min_value / $this->i['graph_max_value']) * ($this->i['graph_left_end'] - $this->c['pos']['left_start'])), 1);
				$value_end_right = $this->c['pos']['left_start'] + round(($max_value / $this->i['graph_max_value']) * ($this->i['graph_left_end'] - $this->c['pos']['left_start']));
				...
				$box_left = $this->c['pos']['left_start'] + round((pts_math::find_percentile($this->graph_data[$i_o][$i], 0.25) / $this->i['graph_max_value']) * ($this->i['graph_left_end'] - $this->c['pos']['left_start']));
				$box_middle = $this->c['pos']['left_start'] + round((pts_math::find_percentile($this->graph_data[$i_o][$i], 0.5) / $this->i['graph_max_value']) * ($this->i['graph_left_end'] - $this->c['pos']['left_start']));
				$box_right = $this->c['pos']['left_start'] + round((pts_math::find_percentile($this->graph_data[$i_o][$i], 0.75) / $this->i['graph_max_value']) * ($this->i['graph_left_end'] - $this->c['pos']['left_start']));
