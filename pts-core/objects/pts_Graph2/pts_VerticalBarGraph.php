...
class pts_VerticalBarGraph extends pts_Graph
{
	...
	protected function render_graph_identifiers()
	{
		...
		$this->svg_dom->draw_svg_line($this->graph_left_start + $this->identifier_width, $this->graph_top_end, $this->graph_left_end - ($this->graph_attr_width % $this->identifier_width), $this->graph_top_end, $this->graph_color_notches, 10, array('stroke-dasharray' => '1,' . ($this->identifier_width - 1)));
		...
		foreach(array_keys($this->graph_identifiers) as $i)
		{
			...
			if($this->graph_font_size_identifiers <= $this->minimum_identifier_font)
			{
				...
				$this->svg_dom->add_text_element($this->graph_identifiers[$i], array('x' => $x, 'y' => $px_from_top_end, 'font-size' => 9, 'fill' => $this->graph_color_headers, 'text-anchor' => 'start', 'dominant-baseline' => 'middle', 'transform' => 'rotate(90 ' . $x . ' ' . $px_from_top_end . ')'));
			}
			else
			{
				$x = $px_bound_left + (($px_bound_right - $px_bound_left) * 0.5);
				$this->svg_dom->add_text_element($this->graph_identifiers[$i], array('x' => $x, 'y' => ($px_from_top_end - 3), 'font-size' => $this->graph_font_size_identifiers, 'fill' => $this->graph_color_headers, 'text-anchor' => 'middle', 'dominant-baseline' => 'text-before-edge'));
			}
		}
	}
	protected function render_graph_bars()
	{
		...
		for($i_o = 0; $i_o < $bar_count; $i_o++)
		{
			...
			foreach(array_keys($this->graph_data[$i_o]) as $i)
			{
				...
				$this->svg_dom->add_element('rect', array('x' => ($px_bound_left + 1), 'y' => $value_plot_top, 'width' => $bar_width, 'height' => ($this->graph_top_end - $value_plot_top), 'fill' => (in_array($this->graph_identifiers[$i], $this->value_highlights) ? $this->graph_color_alert : $paint_color), 'stroke' => $this->graph_color_body_light, 'stroke-width' => 1, 'xlink:title' => $title_tooltip));
				...
				if($graph_size > 18)
				{
					$this->svg_dom->add_text_element($value, array('x' => $x, 'y' => ($value_plot_top + 2), 'font-size' => floor($this->graph_font_size_bars * 0.9), 'fill' => $this->graph_color_body_text, 'text-anchor' => 'middle', 'dominant-baseline' => 'text-before-edge'));
				}
				else
				{
					// Make things more compact
					$this->svg_dom->add_text_element($value, array('x' => $x, 'y' => ($value_plot_top + 2), 'font-size' => floor($this->graph_font_size_bars * 0.6), 'fill' => $this->graph_color_body_text, 'text-anchor' => 'middle', 'dominant-baseline' => 'text-before-edge'));
				}
			}
		}
