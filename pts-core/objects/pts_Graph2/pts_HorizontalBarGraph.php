...
class pts_HorizontalBarGraph extends pts_Graph
{
	...
	protected function render_graph_identifiers()
	{
		...
		$this->svg_dom->draw_svg_line($this->graph_left_start, $this->graph_top_start + $this->identifier_height, $this->graph_left_start, $this->graph_top_end - ($this->graph_attr_height % $this->identifier_height), $this->graph_color_notches, 10, array('stroke-dasharray' => 1 . ',' . ($this->identifier_height - 1)));
		...
		foreach(array_keys($this->graph_identifiers) as $i)
		{
			...
			if($multi_way)
			{
				foreach(explode(' - ', $this->graph_identifiers[$i]) as $i => $identifier_line)
				{
					...
					$this->svg_dom->add_text_element($identifier_line, array('x' => $x, 'y' => $middle_of_vert, 'font-size' => $this->graph_font_size_identifiers, 'fill' => $this->graph_color_headers, 'text-anchor' => 'middle', 'dominant-baseline' => 'text-before-edge', 'transform' => 'rotate(90 ' . $x . ' ' . $middle_of_vert . ')'));
				}
			}
			else
			{
				$this->svg_dom->add_text_element($this->graph_identifiers[$i], array('x' => ($this->graph_left_start - 5), 'y' => $middle_of_vert, 'font-size' => $this->graph_font_size_identifiers, 'fill' => $this->graph_color_headers, 'text-anchor' => 'end', 'dominant-baseline' => 'middle'));
			}
		}
	}
	protected function render_graph_bars()
	{
		...
		for($i_o = 0; $i_o < $bar_count; $i_o++)
		{
			...
			foreach(array_keys($this->graph_data[$i_o]) as $i)
			{
				...
				$graph_size = max(0, round(($value / $this->graph_maximum_value) * ($this->graph_left_end - $this->graph_left_start)));
				...
				$this->svg_dom->add_element('rect', array('x' => $this->graph_left_start, 'y' => $px_bound_top, 'width' => $graph_size, 'height' => $bar_height, 'fill' => (in_array($this->graph_identifiers[$i], $this->value_highlights) ? $this->graph_color_highlight : $paint_color), 'stroke' => $this->graph_color_body_light, 'stroke-width' => 1, 'xlink:title' => $title_tooltip));
				...
				if($std_error != -1 && $value != null)
				{
					...
					if($std_error > 0 && is_numeric($std_error))
					{
						...
						{
							$this->svg_dom->draw_svg_line(($value_end_right - $std_error_rel_size), $px_bound_top, ($value_end_right - $std_error_rel_size), $px_bound_top + $std_error_height, $this->graph_color_notches, 1);
							$this->svg_dom->draw_svg_line(($value_end_right + $std_error_rel_size), $px_bound_top, ($value_end_right + $std_error_rel_size), $px_bound_top + $std_error_height, $this->graph_color_notches, 1);
							$this->svg_dom->draw_svg_line(($value_end_right - $std_error_rel_size), $px_bound_top, ($value_end_right + $std_error_rel_size), $px_bound_top, $this->graph_color_notches, 1);
						}
					}

					$bar_offset_34 = $middle_of_bar + ($multi_way ? 0 : ($bar_height / 5) + 4);
					$this->svg_dom->add_text_element('SE +/- ' . pts_math::set_precision($std_error, 2), array('x' => ($this->graph_left_start - 5), 'y' => $bar_offset_34, 'font-size' => ($this->graph_font_size_identifiers - 2), 'fill' => $this->graph_color_text, 'text-anchor' => 'end', 'dominant-baseline' => 'middle'));
				}

				if(($this->text_string_width($value, $this->graph_font, $this->graph_font_size_identifiers) + 2) < $graph_size)
				{
					$this->svg_dom->add_text_element($value, array('x' => ($value_end_right - 5), 'y' => $middle_of_bar, 'font-size' => $this->graph_font_size_identifiers, 'fill' => $this->graph_color_body_text, 'text-anchor' => 'end', 'dominant-baseline' => 'middle'));
				}
				else if($value > 0)
				{
					// Write it in front of the result
					$this->svg_dom->add_text_element($value, array('x' => ($value_end_right + 6), 'y' => $middle_of_bar, 'font-size' => $this->graph_font_size_identifiers, 'fill' => $this->graph_color_text, 'text-anchor' => 'start', 'dominant-baseline' => 'middle'));
				}
			}
		}
