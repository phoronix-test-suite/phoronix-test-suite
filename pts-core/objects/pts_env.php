...
class pts_env
{
	...
	protected static $env_vars = array(
		...
		'MONITOR' => array(
			...
			'enum' => array('all', 'cpu.peak-freq', 'cpu.temp', 'cpu.power', 'cpu.usage', 'gpu.freq', 'gpu.power', 'gpu.temp', 'hdd.temp', 'memory.usage', 'swap.usage', 'sys.power', 'sys.temp', 'powenetics.power'),
		...
		'POWENETICS' => array(
			'description' => 'This option enables per-rail power measurement support using the Cybenetics/Hardware Busters Powenetics v2 multi-channel PC power measurement device (PMD). The value should be the serial device path (e.g. /dev/ttyUSB0 or COM5), "auto" for auto-discovery of the connected device, or the path to a replay-stream file for testing without hardware. When enabled during benchmarking, the powenetics module captures per-rail power (ATX / EPS / PCIe / PCIe slot) and injects per-rail power consumption graphs into the result file. This works on both Linux and Windows.',
			'default' => '',
			'usage' => array('benchmark'),
			'value_type' => 'string',
			'module' => 'powenetics',
		...
		'POWENETICS_RAILS' => array(
			'description' => 'Used in conjunction with the POWENETICS option, this specifies which rails / aggregates to graph as a comma-separated list. The available aggregates are cpu (EPS total), gpu (PCIe 12V plus PCIe slot), and total (all rails), plus the individual rail tokens. The default value is "cpu,gpu,total" and a value of "all" will graph every individual rail as well as the aggregates.',
			'default' => 'cpu,gpu,total',
			'usage' => array('benchmark'),
			'value_type' => 'string',
			'module' => 'powenetics',
			'advertise_in_phoromatic' => true,
			),
		'POWENETICS_PERF_PER_WATT' => array(
			'description' => 'Used in conjunction with the POWENETICS option, when this value is set a performance-per-Watt graph will be added to the result file using the average total system power as measured by the Powenetics v2 device.',
			'default' => false,
			'usage' => array('benchmark'),
			'value_type' => 'bool',
			'module' => 'powenetics',
		...
		'POWENETICS_INTERVAL' => array(
			'description' => 'Used in conjunction with the POWENETICS option, this specifies the downsampling bucket size in milliseconds for the captured per-rail power measurements. The Powenetics v2 device streams at approximately 1000 samples per second and the captured samples are averaged into buckets of this size. The default value is 100 (milliseconds) and the minimum value is 10.',
			'default' => 100,
			'usage' => array('benchmark'),
			'value_type' => 'positive_integer',
			'module' => 'powenetics',
