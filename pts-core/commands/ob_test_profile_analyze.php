...
var_dump($screenshots);
				foreach($screenshots as $ss_file)
				{
					...
					if(count($ss_delta) < floor(imagesx($reference_ss) / 2) || filesize($ss_file) > 2097152)
					{
			...
			if(is_executable($test_binary))
			{
				...
				foreach(explode(PHP_EOL , shell_exec('objdump -d ' . $test_binary . ' | cut -f3 | cut -d\' \' -f1')) as $instruction)
				{
